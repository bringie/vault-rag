...
**Goal:** Ship vt task tracker as part of vault-rag: 9 REST endpoints (`/api/task/*`) + 9 MCP tools (`task_*`) + thin REST CLI client. Tasks remain markdown in `/vault/04-tasks/`.
...
function tmpVault() {
  ...
  fs.mkdirSync(path.join(d, '04-tasks'), { recursive: true });
...
test('task_create writes file and returns id', async () => {
  ...
  assert.match(res.body.path, /04-tasks\/vt-0001-first-epic\.md$/);
...
function cfgFor(vault) {
  return { tasksDir: path.join(vault, '04-tasks'), seqFile: path.join(vault, '.vt', 'seq') };
}
...
test('task_import refuses without env flag', async () => {
  ...
  const res = await handlers.import_task({ vault, body: { path: '04-tasks/vt-0099-x.md', content: '---\nid: vt-0099\n---\n' } });
...
test('task_import writes file and bumps seq', async () => {
  ...
  const res = await handlers.import_task({ vault, body: { path: '04-tasks/vt-0042-x.md', content: md } });
  assert.strictEqual(res.status, 200);
...
test('task_import refuses overwrite', async () => {
  ...
  process.env.VAULT_RAG_ALLOW_IMPORT = '1';
  const md = '---\nid: vt-0001\ntitle: x\ntype: task\nstatus: open\npriority: 2\ncreated: 2026-01-01T00:00:00.000Z\n---\n';
  await handlers.import_task({ vault, body: { path: '04-tasks/vt-0001-x.md', content: md } });
  const res = await handlers.import_task({ vault, body: { path: '04-tasks/vt-0001-x.md', content: md } });
  assert.strictEqual(res.status, 409);
  delete process.env.VAULT_RAG_ALLOW_IMPORT;
});
```
...
async function import_task({ vault, body }) {
  ...
  if (!relPath.startsWith('04-tasks/') || relPath.includes('..')) {
    return { status: 400, body: { error: 'path must start with 04-tasks/ and contain no ..' } };
  }
...
mkdir -p /tmp/vault-test/04-tasks /tmp/vault-test/.vt
...
curl -s -X POST http://127.0.0.1:15679/task/create \
  ...
  -d '{"title":"smoke"}'
# Expected: {"id":"vt-0001","path":"04-tasks/vt-0001-smoke.md"}
...
`vt remember` writes to `06-resources/notes/` which is currently not a writable prefix. Add it so the rewritten CLI can post via existing `/api/put`.

- [ ] **Step 1: Locate current WRITABLE_PREFIXES**

Run: `grep -n "WRITABLE_PREFIXES" /root/work/vault-rag-oss/scripts/rag-api.js`

- [ ] **Step 2: Add `06-resources/notes/`**
...
(`04-tasks/` added so any future direct put works the same way; tasks themselves go through `/task/*` routes.)

- [ ] **Step 3: Smoke**

```bash
cd /root/work/vault-rag-oss
mkdir -p /tmp/vault-test/06-resources/notes /tmp/vault-test/04-tasks /tmp/vault-test/.vt
VAULT_PATH=/tmp/vault-test VAULT_RAG_API_TOKEN=test PORT=15679 node scripts/rag-api.js &
sleep 1
curl -s -X POST http://127.0.0.1:15679/put -H "Authorization: Bearer test" \
  -H "Content-Type: application/json" \
  -d '{"path":"06-resources/notes/test.md","content":"---\ntype: note\n---\nhi","reindex":false}'
# Expected: success
kill %1
rm -rf /tmp/vault-test
```
...
```bash
git add scripts/rag-api.js
git commit -m "feat(rag-api): allow writes to 06-resources/notes and 04-tasks"
```
...
const cmds = {
  ...
  async remember(args) {
    ...
    const res = await fetch(`${c.url}/api/put`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${c.token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: `06-resources/notes/${stamp}-${slug}.md`, content: fm, mode: 'create', reindex: false }),
    });
...
```bash
cd /root/work/vault-rag-oss
mkdir -p /tmp/vault-test/04-tasks /tmp/vault-test/06-resources/notes /tmp/vault-test/.vt
VAULT_PATH=/tmp/vault-test VAULT_RAG_API_TOKEN=test PORT=15679 node scripts/rag-api.js &
sleep 1
...
set -euo pipefail
VT_VAULT=$(mktemp -d)
mkdir -p "$VT_VAULT/04-tasks" "$VT_VAULT/.vt" "$VT_VAULT/06-resources/notes"
...
Tests like `[ -f "$TMPDIR_TEST/04-tasks/vt-0001-first-epic.md" ]` become `[ -f "$VT_VAULT/04-tasks/vt-0001-first-epic.md" ]`. Tests that grep frontmatter still work. Test `missing vault dir errors` is replaced with `unreachable server errors`:
...
```bash
cd /root/work/vault-rag-oss
mkdir -p /tmp/v-src/04-tasks /tmp/v-dst/04-tasks /tmp/v-dst/.vt
echo -e '---\nid: vt-0001\ntitle: a\ntype: task\nstatus: open\npriority: 2\ncreated: 2026-01-01T00:00:00.000Z\n---\nbody' > /tmp/v-src/04-tasks/vt-0001-a.md
...
Tasks live as markdown in `<vault>/04-tasks/vt-NNNN-slug.md`. Counter at `<vault>/.vt/seq`. Edit by hand if needed.
...
| `vt remember "note" [--tags a,b]` | Save note to `06-resources/notes/`. |
...
vault-rag ships with `vt`, a task tracker. Tasks are markdown files in `<vault>/04-tasks/` indexed alongside the rest of the vault.
...
ssh -p 977 root@brain.itiswednesdaymydud.es 'ls /root/obsidian-vault/04-tasks/'
...
- Indexer churn -> existing incremental indexer already reads `04-tasks/*.md`; no change needed.
